...
# Sheet ID desde secrets (nube) o valor por defecto (local)
try:
    SHEET_ID = st.secrets["sheet_id"]
except:
    SHEET_ID = "1ay-_hM0Yvte-oWoU4B9qbp0Lt46x0xiuVRAY8Cdhiys"
...
# ── TAB 2: GANTT ──
with tab_gantt:
    st.subheader("📊 Gantt — Trabajos pendientes")

    if st.session_state.vehicles:
        # Selector de rango + filtros en la misma fila
        ...
        rango_sel = r1.selectbox("Rango de tiempo:", list(rango_opciones.keys()), index=1, key="gantt_rango")
        filtro_op_g = r2.multiselect("Operario:", TEAMS, default=TEAMS, key="gantt_op")
        filtro_et_g = r3.multiselect("Etapa:", [s.capitalize() for s in STAGES_ORDER],
                                      default=[s.capitalize() for s in STAGES_ORDER], key="gantt_et")

        dias_rango = rango_opciones[rango_sel]
        fecha_fin_rango = hoy + timedelta(days=dias_rango)

        # Construir datos — solo etapas pendientes desde hoy
        gantt_data = []
        for v in st.session_state.vehicles:
            ...
            for s in STAGES_ORDER:
                ...
                inicio = info['start']
                fin    = info['end']
                # Solo incluir si está dentro del rango y empieza desde hoy
                if fin >= hoy and inicio <= fecha_fin_rango:
                    inicio_vis = max(inicio, hoy)
                    fin_vis    = min(fin, fecha_fin_rango)
                    gantt_data.append({
        ...
        if gantt_data:
            ...
            if not df_filtrado.empty:
                ...
                fig = ff.create_gantt(
                    ...
                    showgrid_x=True,
                    showgrid_y=True,
                    title=f"Próximos {rango_sel} — {hoy.strftime('%d/%m/%Y')} en adelante"
                )
                ...
                if dias_rango < 9999:
                    fig.update_xaxes(range=[
                        ...
                        datetime.combine(fecha_fin_rango + timedelta(days=1), datetime.min.time())
                    ])
                ...
                st.caption(f"{len(df_filtrado['Task'].unique())} OTs activas en este período")
            else:
                st.info("No hay OTs activas en este período con los filtros seleccionados.")
        else:
