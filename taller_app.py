...
def importar_planilla(df_raw):
    ...
    for _, row in df_raw.iterrows():
        try:
            ...
            # Fecha ingreso (TALLER) es la base — si no existe, calcular tentativa
            try:
                fi_raw = pd.to_datetime(row.get('TALLER'))
                if pd.isna(fi_raw):
                    raise ValueError
                fi = fi_raw.date()
                fecha_tentativa = False
            except:
                # Sin fecha de taller: calcular tentativa según carga actual
                fi = hoy
                fecha_tentativa = True

            # Fecha cliente es referencial — no se usa para calcular atrasos
            try:
                fe_raw = pd.to_datetime(row.get('CLIENTE'))
                fe_str = fe_raw.strftime('%Y-%m-%d') if not pd.isna(fe_raw) else ''
            except:
                fe_str = ''
...
ots = load_ots()
n_urgentes = sum(1 for o in ots if dias_restantes(o['fecha_ingreso']) is not None
                 and dias_restantes(o['fecha_ingreso']) <= 3
...
with tab_hoy:
    ...
    # Alertas urgentes
    urgentes = [o for o in ots if dias_restantes(o['fecha_ingreso']) is not None
                and dias_restantes(o['fecha_ingreso']) <= 3
                and o['etapa_actual'] != 'listo']
    if urgentes:
        st.markdown("#### 🚨 Urgentes")
        for u in urgentes:
            dr = dias_restantes(u['fecha_ingreso'])
