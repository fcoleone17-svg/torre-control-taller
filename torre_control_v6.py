...
SHEET_ID = "1Oz54r7P5T3WP_XsEZHB8uxNnkJ8ZrWssC01dqxC0j9I"
...
def get_gsheet_client():
    if not GSPREAD_AVAILABLE:
        st.session_state['gsheet_error'] = "gspread no instalado"
        return None
    try:
        if is_cloud():
...
# ── TAB 2: GANTT ──
with tab_gantt:
    st.subheader("📊 Gantt Semanal")

    if st.session_state.vehicles:
        # Navegación semanal
        if 'gantt_semana' not in st.session_state:
            st.session_state.gantt_semana = 0

        lunes_gantt = hoy - timedelta(days=hoy.weekday()) + timedelta(weeks=st.session_state.gantt_semana)
        viernes_gantt = lunes_gantt + timedelta(days=4)

        nav1, nav2, nav3, nav4 = st.columns([1, 1, 3, 2])
        if nav1.button("◀ Anterior", key="gantt_prev"):
            st.session_state.gantt_semana -= 1
            st.rerun()
        if nav2.button("Siguiente ▶", key="gantt_next"):
            st.session_state.gantt_semana += 1
            st.rerun()
        if nav3.button("📅 Semana actual", key="gantt_hoy"):
            st.session_state.gantt_semana = 0
            st.rerun()
        nav4.markdown(f"**{lunes_gantt.strftime('%d/%m/%Y')} → {viernes_gantt.strftime('%d/%m/%Y')}**")

        # Filtros
        col_f1, col_f2 = st.columns(2)
        filtro_op_g = col_f1.multiselect("Filtrar por operario:", TEAMS, default=TEAMS, key="gantt_op")
        filtro_et_g = col_f2.multiselect("Filtrar por etapa:", [s.capitalize() for s in STAGES_ORDER],
                                          default=[s.capitalize() for s in STAGES_ORDER], key="gantt_et")

        # Construir datos — solo etapas que caen en la semana seleccionada
        gantt_data = []
        for v in st.session_state.vehicles:
            etapas_ok_g = set(ETAPAS_COMPLETADAS_POR_ESTADO.get(v['Estado'], []))
            for s in STAGES_ORDER:
                info = v['Asignaciones'].get(s)
                if not info:
                    continue
                es_comp = (s in etapas_ok_g or info.get('completado', False)
                           or info['end'] < hoy)
                if es_comp:
                    continue
                ...
                inicio = info['start']
                fin    = info['end']
                # Incluir si hay solapamiento con la semana
                if fin >= lunes_gantt and inicio <= viernes_gantt:
                    # Recortar al rango de la semana para visualización
                    inicio_vis = max(inicio, lunes_gantt)
                    fin_vis    = min(fin, viernes_gantt)
                    gantt_data.append({
                        'Task': f"{v['OT']} — {v['Patente']}",
                        'Start':  datetime.combine(inicio_vis, datetime.min.time()),
                        'Finish': datetime.combine(fin_vis + timedelta(days=1), datetime.min.time()),
        ...
        if gantt_data:
            ...
            df_filtrado = df_gantt[
                df_gantt['Operario'].isin(filtro_op_g) &
                df_gantt['Resource'].isin(filtro_et_g)
            ]
            ...
            if not df_filtrado.empty:
                ...
                fig = ff.create_gantt(
                    ...
                    showgrid_x=True,
                    showgrid_y=True,
                    title=f"Semana {lunes_gantt.strftime('%d/%m')} — {viernes_gantt.strftime('%d/%m/%Y')}"
                )

                # Línea vertical en hoy
                hoy_dt = datetime.combine(hoy, datetime.min.time())
                fig.add_shape(type="line",
                    x0=hoy_dt, x1=hoy_dt, y0=0, y1=1,
                    xref="x", yref="paper",
                    line=dict(color="red", width=2, dash="dash")
                )
                fig.add_annotation(
                    x=hoy_dt, y=1, xref="x", yref="paper",
                    text="Hoy", showarrow=False,
                    font=dict(color="red", size=11),
                    yanchor="bottom"
                )

                # Ajustar rango del eje X a la semana
                fig.update_xaxes(
                    range=[
                        datetime.combine(lunes_gantt, datetime.min.time()),
                        datetime.combine(viernes_gantt + timedelta(days=1), datetime.min.time())
                    ]
                )

                fig.update_layout(
                    height=max(400, len(df_filtrado['Task'].unique()) * 35 + 120),
                    xaxis_title="Fecha", font=dict(size=11)
                )
                st.plotly_chart(fig, use_container_width=True)
                st.caption(f"Mostrando {len(df_filtrado['Task'].unique())} OTs activas esta semana")
            else:
                st.info("No hay OTs activas esta semana con los filtros seleccionados.")
        else:
            st.info("No hay OTs activas esta semana.")
    else:
    ...
    if GSPREAD_AVAILABLE:
        ...
        if gs2.button("🔄 Importar desde Sheets", key="btn_import_sheets"):
            ...
            if df_sheet is not None:
                ...
                err = st.session_state.get('gsheet_error', 'Error desconocido')
                st.error(f"No se pudo conectar con Google Sheets: {err}")
