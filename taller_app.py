...
def save_ot(data):
    ...
    c.execute('''INSERT OR REPLACE INTO ots VALUES
        ...
        data.get('dano','MEDIANO'),
        data['desabollador'], data['pintor'],
        data['etapa_actual'],
        data.get('fecha_ingreso',''), data.get('fecha_entrega',''),
        data.get('comentario',''), data.get('repuestos',''), data.get('telefono',''),
        json.dumps(data.get('programacion', {})),
        0, data.get('posicion','')
    ))
    conn.commit()
...
def load_ots():
    c = conn.cursor()
    c.execute("SELECT * FROM ots WHERE entregado=0 ORDER BY fecha_entrega")
...
def mejor_desabollador(ots):
    """Elige el desabollador con menos carga en próximos 14 días"""
    carga = {d: 0 for d in DESABOLLADORES}
    ...
    return min(carga, key=carga.get)

def mejor_pintor(ots):
    carga = {p: 0 for p in PINTORES}
    fin_14 = hoy + timedelta(days=14)
    for ot in ots:
        prog = ot.get('programacion', {})
        pin = ot.get('pintor', '')
...
def importar_planilla(df_raw):
    ...
    for _, row in df_raw.iterrows():
        try:
            ...
            try:
                fi = pd.to_datetime(row.get('TALLER')).date()
            except:
                fi = hoy

            try:
                fe = pd.to_datetime(row.get('CLIENTE')).date()
...
if is_cloud() and supa:
    st.sidebar.success("☁️ BD: Supabase")
elif is_cloud() and not supa:
    st.sidebar.error("⚠️ Cloud pero sin Supabase")
else:
    st.sidebar.info("💻 BD: Local (SQLite)")

ots = load_ots()
n_urgentes = sum(1 for o in ots if dias_restantes(o['fecha_entrega']) is not None
                 and dias_restantes(o['fecha_entrega']) <= 3
...
with tab_hoy:
    ...
    # Alertas urgentes
    urgentes = [o for o in ots if dias_restantes(o['fecha_entrega']) is not None
                and dias_restantes(o['fecha_entrega']) <= 3
                and o['etapa_actual'] != 'listo']
    if urgentes:
...
with tab_panel:
    ...
    st.caption(f"Mostrando {len(ots_filtradas)} de {len(ots)} vehículos")
    st.markdown("---")
    ...
    for ot in ots_filtradas:
        ...
        if dr is not None:
            if dr < 0:
                fecha_label = f"<span style='color:#c53030;font-weight:700'>🚨 {format_fecha(ot['fecha_entrega'])} ({abs(dr)}d atraso)</span>"
            elif dr <= 3:
                fecha_label = f"<span style='color:#dd6b20;font-weight:700'>⚠️ {format_fecha(ot['fecha_entrega'])} ({dr}d)</span>"
            else:
                fecha_label = f"<span style='color:#666'>{format_fecha(ot['fecha_entrega'])}</span>"
        else:
            fecha_label = "<span style='color:#999'>Sin fecha</span>"
        ...
        with col_info:
            ...
            <div style='background:{row_bg};padding:12px;border-radius:10px;border:1px solid #e8e8e8;height:100%'>
                ...
                <div style='font-size:12px;color:#666;margin-top:2px'>{ot['nombre'] or '—'} · {ot['cia'] or '—'}</div>
                <div style='margin-top:6px'>{fecha_label}</div>
                <div style='font-size:12px;color:#888;margin-top:4px'>
                    Des: <b>{ot['desabollador']}</b> · Pin: <b>{ot['pintor']}</b>
                    {'· 📍 <b>' + ot.get('posicion','') + '</b>' if ot.get('posicion') else ''}
                </div>
            </div>
            """, unsafe_allow_html=True)
        ...
        if st.session_state.editar_ot == ot['ot']:
            with st.container():
                ...
                if s1.button("💾 Guardar", key=f"save_{ot['ot']}", type="primary"):
                    ...
                    fields = {'etapa_actual': nueva_etapa, 'desabollador': nuevo_des,
                              'pintor': nuevo_pin, 'comentario': nuevo_com,
                              'repuestos': nuevo_rep, 'posicion': pos_guardar}
                    if supa:
                        supa.table("ots").update(fields).eq("ot", ot['ot']).execute()
                    else:
                        c = conn.cursor()
...
with tab_nueva:
    ...
    with st.form("form_nueva_ot"):
        ...
        ots_actual = load_ots()
        if auto_asignar:
            sug_des = mejor_desabollador(ots_actual)
            sug_pin = mejor_pintor(ots_actual)
...
with tab_config:
    ...
    with st.expander("⚠️ Zona de peligro"):
        if st.button("🗑️ Eliminar TODOS los vehículos", type="secondary"):
            if supa:
