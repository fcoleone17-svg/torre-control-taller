...
with tab_panel:
    ...
    for ot in ots_filtradas:
        ...
        if tentativa:
            ...
            if not es_listo and dr is not None and dr < 0:
                fecha_label = f"<span style='color:#c53030;font-weight:700'>🚨 Ingreso: {fi_fmt} ({abs(dr)}d atraso)</span>"
            elif not es_listo and dr is not None and dr <= 3:
                fecha_label = f"<span style='color:#dd6b20;font-weight:700'>⚠️ Ingreso: {fi_fmt} ({dr}d)</span>"
            else:
                fecha_label = f"<span style='color:#444'>🔧 Ingreso: {fi_fmt}</span>"
